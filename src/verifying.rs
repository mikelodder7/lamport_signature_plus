...
///
/// A public key is generally derived from its paired [`SigningKey`] or returned
/// by [`generate_keys`](crate::generate_keys).
...
impl<T: LamportDigest> VerifyingKey<T> {
    /// Construct a [`VerifyingKey`] from a byte sequence.
    #[cfg_attr(coverage_nightly, coverage(off))]
