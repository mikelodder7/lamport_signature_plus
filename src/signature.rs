...
use crate::utils::{encode_slices, prefixed_share};
...
/// A signature generated by [`SigningKey`](crate::SigningKey).
...
impl<T: LamportDigest> Signature<T> {
    /// Construct a [`Signature`] from a byte sequence.
    #[cfg_attr(coverage_nightly, coverage(off))]
    ...
    pub(crate) fn combine_refs(shares: &[&SignatureShare<T>]) -> LamportResult<Self> {
        if shares.is_empty() {
            return Err(LamportError::InvalidSignatureBytes);
        }
        let first = &shares[0];
...
impl<T: LamportDigest> SignatureShare<T> {
    /// Construct a [`SignatureShare`] from a byte sequence.
    #[cfg_attr(coverage_nightly, coverage(off))]
    pub fn from_bytes<B: AsRef<[u8]>>(bytes: B) -> LamportResult<Self> {
        let bytes = bytes.as_ref();
        let digest_size_in_bits = T::digest_size_in_bits();
